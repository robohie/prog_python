...
class TensionVoice:
    ...
    """
    def __init__(self, f:float, *amplitudes):
        """

        :param f: fréquence fondamentale (Hz)
        :param amplitudes: les amps variables (des fonctions)
        """
        if not 20<=f<=2000:
    ...
    def get_signal(self, x):
        ...
        for k, env in enumerate(self.amplitudes, start=1):
...
class CapacityVariable(TensionVoice):
    ...
    def get_signal(self, x):
        ...
        self.f, ayant des amps et de déphasages donnés

        :param x: temps (s)
        """
        x = np.asarray(x)
        s = np.zeros_like(x, dtype=float)
        for k, (capa, phase) in enumerate(zip(self.amplitudes, self.phases), start=1):
